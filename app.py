...
#flowers-quiz *{box-sizing:border-box}
...
.visual{
  ...
  border-radius:24px;
  background:
    linear-gradient(180deg,#eaf8ff 0%,#fff7c5 70%,#d1e9aa 71%,#8ebe5c 100%);
}
.sun{
  position:absolute;
  right:27px;top:23px;
  ...
  border-radius:50%;
  background:#ffe15a;
  box-shadow:0 0 34px rgba(255,202,0,.38);
}
.cloud{
  position:absolute;
...
.c1{left:8%;top:54px}
.c2{right:20%;top:86px;transform:scale(.76)}
...
.paper{
  ...
  background:linear-gradient(145deg,#fff9e1,#e5c768);
  filter:drop-shadow(0 16px 14px rgba(90,62,0,.13));
  z-index:2;
}
.ribbon{
  position:absolute;
...
.flower{
  ...
  filter:drop-shadow(0 8px 7px rgba(76,52,0,.13));
...
.pet{
  ...
  background:linear-gradient(#ffec7a,#ffc817);
...
.center{
  ...
  border-radius:50%;
  background:radial-gradient(circle at 35% 30%,#a46e18,#69400c 72%);
  z-index:5;
}
...
.startBtn{
  ...
  padding:12px 14px;
  border-radius:14px;
  background:linear-gradient(90deg,#ffc817,#ffe774);
  color:#594500;
  font-weight:900;
  cursor:pointer;
}
...
.option.selected{
  background:#fff0a1;
  border-color:#cfa619;
  box-shadow:0 0 0 2px rgba(207,166,25,.12);
}
.feedback{
  margin-top:10px;
  min-height:48px;
...
.final{
  ...
  border:1px solid #dec45d;
  border-radius:24px;
  background:linear-gradient(160deg,#fffef3,#fff0a4);
  padding:18px;
  text-align:center;
}
...
@keyframes fall{
  to{transform:translateY(560px) rotate(500deg);opacity:.1}
}
...
<section class="wrap">
  <header class="top">
    ...
    <p>
      Un detalle por el día de las flores amarillas. Primero mira tu ramo y luego responde unas preguntas.
    </p>
  </header>
  ...
  <div class="hero">
    ...
    <aside class="intro-card" id="introCard">
      ...
      <p>Marca lo que más te representa en unas preguntas cortitas. Al terminar se desbloquea un mensaje especial.</p>
  ...
  <section class="quiz" id="quiz">
    <div class="quiz-head">
      <h2 id="quizTitle">Unas preguntas rápidas</h2>
  ...
  <section class="final" id="final">
    ...
    <p>
      ...
      Te dejo este ramo porque una buena amistad también merece detalles bonitos.
      Gracias por las risas, las conversaciones, la confianza y por esos momentos simples
      que terminan siendo buenos recuerdos.<br><br>
      <b>Que hoy te sobren motivos para sonreír y que sigan llegando buenos momentos. ✨</b>
    </p>
...
<script>
(() => {
  const root=document.getElementById("flowers-quiz");
  ...
  const questions=[
    {
      ...
      fb:[
        "Las risas hacen que cualquier momento sea mejor 😄",
        "Eso vale muchísimo en una amistad 🤝",
        "Una buena conversación siempre suma ☕",
        "Exacto: un poquito de todo hace la diferencia ✨"
      ]
    },
    {
      ...
      fb:[
        ...
        "Comida + conversación = buen plan 😄",
        "Simple, tranquilo y buena onda 🚶",
        "Esa respuesta resume todo 🙌"
      ]
    },
    {
      ...
      fb:[
        "A veces un mensaje cambia el día 📱",
        "Esas son las risas que más se recuerdan 😂",
        "Ese tipo de presencia vale bastante 🤝",
        "Como estas flores amarillas de hoy 🌻"
      ]
    },
    {
      ...
      fb:[
        ...
        "Una amistad sincera vale muchísimo 🤝",
        "Esa era la mejor respuesta para cerrar 🌻"
      ]
    }
  ];
  ...
  startBtn.addEventListener("click",()=>{
    ...
    introCard.innerHTML="<div class='big'>🌼</div><h2>Tu ramo ya está listo</h2><p>Ahora responde las preguntas de abajo y al final aparecerá el mensaje.</p>";
    render();
